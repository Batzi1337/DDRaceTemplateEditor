...
func TestHammer(t *testing.T) {
	srcImage := getImage("../../samples/templates/template1.png")
	src := Template(srcImage)

	item := Hammer(src)

	// Verify that the item has the correct dimensions
	assert.Equal(t, 128, item.Bounds().Dx())
	assert.Equal(t, 96, item.Bounds().Dy())
}
...
func TestShotgun(t *testing.T) {
	...
	// Verify that the item has the correct dimensions
	assert.Equal(t, 256, item.Bounds().Dx())
	assert.Equal(t, 64, item.Bounds().Dy())
}

func TestShotgunCrosshair(t *testing.T) {
	srcImage := getImage("../../samples/templates/template1.png")
	src := Template(srcImage)

	item := ShotgunCrosshair(src)

	// Verify that the item has the correct dimensions
	assert.Equal(t, 64, item.Bounds().Dx())
	assert.Equal(t, 64, item.Bounds().Dy())
}

func TestShotgunBullet(t *testing.T) {
	srcImage := getImage("../../samples/templates/template1.png")
	src := Template(srcImage)

	item := ShotgunBullet(src)
...
func TestPistolCrosshair(t *testing.T) {
	srcImage := getImage("../../samples/templates/template1.png")
	src := Template(srcImage)

	item := PistolCrosshair(src)
...
func TestPistolBullet(t *testing.T) {
	srcImage := getImage("../../samples/templates/template1.png")
	src := Template(srcImage)

	item := PistolBullet(src)
